...
This bar graph displays the frequency of each category of household income.  We can clearly see that the majority of respondents come from households of medium and high income.

The below graphs depict attitude toward abortion in relation to various demographic metrics.
...
The above graph shows the number of respondents that have/have not had an abortion (or know someone that has), grouped by education level.  There does seem to be a general trend in that there are more respondents of higher education rates that have not/do not know of anyone who had an abortion.  However, this is also the case for lower and medium education levels as well, so it is not definitive as to whether or not these two variables are strongly correlated.
...
This graph is somewhat surprising, as we had anticipated a stronger relationship between gender and attitude for females.
...
This graph is illuminating, as we had not anticipated Europe being so pro-abortion when compared to other continents.  There appears to be a trend here, which we further evaluate later.
...
This graph demonstrated a correlation between rates of education and attitudes toward abortion.  As seen above, those with higher rates of education seem to be more favorable to abortion.  That being said, this  graph also reveals the lower response rate among persons with lower education.  As such, we will further examine the cross-section of attitude and education to understand how significant the correlation is.

```{r}
counts_abortion <- table(data$had_abortion, data$attitude)
barplot(counts_abortion, main="Attitude by Whether The Respondent or Someone Close to Respondent has had an Abortion",
...
This graph also does not suggest a relationship between whether or not the respondent is the chief income earner.
...
This graph shows the number of people in favor/not in favor of the legalization of abortion, categorized by their age and further broken down into their gender.  As previously observed, there is a relatively even split between male and female for each attitude (legal / illegal), and this did not change when the subjects were divided by age.  It is once again surprising to see the lack of a stronger correlation between these variables and attitude towards abortion.
...
```
```{r}
# visualize the re-sample distributions and variable importance 
...
```{r console = FALSE, message=FALSE, warning=FALSE}
...
We chose the tuned decision tree model as our "Final" model to move foward with. We evaluated the performance of the model using the test dataset. 
...
abort_dtFINAL_eval
```
When the final model, we observed similar model metric performance as with the tune dataset. The F1 score was quite high at 0.9119, which was likely due to the very high sensitivity of the model. The model had a sensitivity of 0.9775 on the test dataset, meaning that about 98% of all positive observations (legal) were properly classified. However, as we have consistently observed the model performed fairly poorly on the negative class with a specificity of 0.1509, and a corresponding false positive rate of about 95%. 
...
Finally, is important to remember that variables such as gender, household income, and age are protected classes. However, we decided against conducting a fairness assessment because in the context of our question we felt it was unnecessary because no action we take (targeting people for ads/outreach) would disadvantage these protected classes.
